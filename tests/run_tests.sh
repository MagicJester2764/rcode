...
roundtrip "rc1 single-cipher (tqbf)"  "$FIX/rc1.cps"   inline:1       "$FIX/tqbf.txt"
roundtrip "rc2 single-cipher (tqbf)"  "$FIX/rc2.cps"   inline:1       "$FIX/tqbf.txt"
roundtrip "rc3 single-cipher (tqbf)"  "$FIX/rc3.cps"   inline:1       "$FIX/tqbf.txt"
roundtrip "rc123 mixed pattern 132312" "$FIX/rc123.cps" inline:132312 "$FIX/tqbf.txt"
roundtrip "mixed pattern from file"   "$FIX/rc123.cps" "$FIX/pat132312.ptn" "$FIX/tqbf.txt"
...
exact_output "mixed KAT == RCode.txt line 23" "$FIX/rc123.cps" inline:132312 "$FIX/tqbf.txt" "$TMP/expect"
printf 'qb? 6e13v !.9ud 098 %se4f2 9y?. qb? hgjc 795\n' "'" >"$TMP/expect"
exact_output "rc1 KAT == RCode.txt line 11" "$FIX/rc1.cps" inline:1 "$FIX/tqbf.txt" "$TMP/expect"
printf '_2o yr,%sz 3u64j g6? hrkl7 6"ou _2o tq5f p6x\n' "'" >"$TMP/expect"
exact_output "rc3 KAT == RCode.txt line 17" "$FIX/rc3.cps" inline:1 "$FIX/tqbf.txt" "$TMP/expect"

# no-op guard: a real cipher must change the bytes (catches identity/no-op regressions)
"$RCODE" encrypt --ciphers "$FIX/rc1.cps" --pattern inline:1 --in "$FIX/tqbf.txt" --out "$TMP/ct" 2>"$TMP/e"
...
printf 'hi' >"$TMP/short"
roundtrip "pattern longer than input" "$FIX/n3.cps" inline:123123123 "$TMP/short"
roundtrip "input longer than pattern (wrap)" "$FIX/n3.cps" inline:12 "$FIX/tqbf.txt"

# ---- all 94 symbols incl both A and a (case significance) ------------
roundtrip "all 94 symbols (A != a)" "$FIX/rc123.cps" inline:132312 "$FIX/all94.txt"

# identity cipher: ciphertext must equal plaintext exactly (exact-output)
exact_output "identity cipher is a no-op (exact output)" "$FIX/identity.cps" inline:1 "$FIX/all94.txt" "$FIX/all94.txt"

# ---- N boundaries: 1, 3, 9, 10, 50 -----------------------------------
roundtrip "N=1"  "$FIX/n1.cps"  inline:1            "$FIX/tqbf.txt"
roundtrip "N=3"  "$FIX/n3.cps"  inline:132          "$FIX/tqbf.txt"
roundtrip "N=9"  "$FIX/n9.cps"  inline:132312       "$FIX/tqbf.txt"
roundtrip "N=10 (separated indices)" "$FIX/n10.cps" "inline:1 5 10 3" "$FIX/tqbf.txt"
roundtrip "N=50 (separated indices)" "$FIX/n50.cps" "inline:50 1 25 7" "$FIX/tqbf.txt"
...
roundtrip "N=9 accepts bare digit string 132312" "$FIX/n9.cps" inline:132312 "$FIX/tqbf.txt"
expect_exit "N=10 rejects bare digit string 132312 (out of range run)" 2 \
    "$RCODE" encrypt --ciphers "$FIX/n10.cps" --pattern inline:132312 --in "$FIX/tqbf.txt"
expect_exit "N=10 rejects bare digit string 11 (in range)" 2 \
    "$RCODE" encrypt --ciphers "$FIX/n10.cps" --pattern inline:11 --in "$FIX/tqbf.txt"
expect_stderr "  ^ rejected as bare digit string" "bare digit string"
expect_exit "N=50 rejects bare digit string 25 (in range)" 2 \
    "$RCODE" encrypt --ciphers "$FIX/n50.cps" --pattern inline:25 --in "$FIX/tqbf.txt"
roundtrip "N=10 accepts separated multi-digit (1 10)" "$FIX/n10.cps" "inline:1 10" "$FIX/tqbf.txt"
roundtrip "N=10 accepts lone single-digit index (5)" "$FIX/n10.cps" inline:5 "$FIX/tqbf.txt"

# duplicate index values are legal
roundtrip "duplicate index pattern (111)" "$FIX/n3.cps" inline:111 "$FIX/tqbf.txt"

# ---- whitespace passthrough / CRLF / trailing newline / empty --------
roundtrip "whitespace + CRLF passthrough"  "$FIX/rc123.cps" inline:132312 "$FIX/ws.txt"
roundtrip "empty input"                    "$FIX/rc1.cps"   inline:1      "$FIX/empty.txt"
...
expect_exit "reject non-bijection cipher"  2 \
    "$RCODE" encrypt --ciphers "$FIX/bad_nonbijection.cps" --pattern inline:1 --in "$FIX/tqbf.txt"
expect_stderr "  ^ bijection path" "not a bijection"
expect_exit "reject wrong token count"     2 \
    "$RCODE" encrypt --ciphers "$FIX/bad_tokencount.cps" --pattern inline:1 --in "$FIX/tqbf.txt"
expect_stderr "  ^ token-count path" "expected 94 tokens"
expect_exit "reject two-byte token"        2 \
    "$RCODE" encrypt --ciphers "$FIX/bad_twobyte_token.cps" --pattern inline:1 --in "$FIX/tqbf.txt"
expect_stderr "  ^ token-length path" "is not a single byte"
expect_exit "reject non-ASCII cipher byte" 2 \
    "$RCODE" encrypt --ciphers "$FIX/bad_nonascii.cps" --pattern inline:1 --in "$FIX/tqbf.txt"
expect_stderr "  ^ byte-range path" "outside printable range"
expect_exit "reject too-many tokens (95)"  2 \
    "$RCODE" encrypt --ciphers "$FIX/bad_toomany.cps" --pattern inline:1 --in "$FIX/tqbf.txt"
...
expect_exit "reject invalid 2nd cipher line" 2 \
    "$RCODE" encrypt --ciphers "$FIX/bad_second_line.cps" --pattern inline:1 --in "$FIX/tqbf.txt"
expect_stderr "  ^ reports cipher index 2" "cipher 2"
expect_exit "reject out-of-range pattern index" 2 \
    "$RCODE" encrypt --ciphers "$FIX/rc1.cps" --pattern inline:2 --in "$FIX/tqbf.txt"
expect_exit "reject empty pattern"         2 \
    "$RCODE" encrypt --ciphers "$FIX/rc1.cps" --pattern inline: --in "$FIX/tqbf.txt"

expect_exit "reject out-of-alphabet input 0x80" 3 \
    "$RCODE" encrypt --ciphers "$FIX/rc1.cps" --pattern inline:1 --in "$FIX/in_0x80.bin"
expect_exit "reject NUL input 0x00"        3 \
    "$RCODE" encrypt --ciphers "$FIX/rc1.cps" --pattern inline:1 --in "$FIX/in_0x00.bin"
expect_exit "reject control input 0x07"    3 \
    "$RCODE" encrypt --ciphers "$FIX/rc1.cps" --pattern inline:1 --in "$FIX/in_0x07.bin"
expect_exit "reject unwritable --out"      3 \
    "$RCODE" encrypt --ciphers "$FIX/rc1.cps" --pattern inline:1 --in "$FIX/tqbf.txt" --out "$TMP/nodir/out"
expect_exit "reject missing input file"    3 \
    "$RCODE" encrypt --ciphers "$FIX/rc1.cps" --pattern inline:1 --in "$TMP/does_not_exist"

# ---- decrypt-side rejections (isolate the inverse-table/decrypt branch) --
expect_exit "decrypt: reject out-of-alphabet input 0x80" 3 \
    "$RCODE" decrypt --ciphers "$FIX/rc1.cps" --pattern inline:1 --in "$FIX/in_0x80.bin"
expect_exit "decrypt: reject control input 0x07" 3 \
    "$RCODE" decrypt --ciphers "$FIX/rc1.cps" --pattern inline:1 --in "$FIX/in_0x07.bin"
expect_exit "decrypt: reject non-bijection cipher" 2 \
    "$RCODE" decrypt --ciphers "$FIX/bad_nonbijection.cps" --pattern inline:1 --in "$FIX/tqbf.txt"
expect_exit "decrypt: reject out-of-range pattern index" 2 \
    "$RCODE" decrypt --ciphers "$FIX/rc1.cps" --pattern inline:2 --in "$FIX/tqbf.txt"

# ---- default stdin/stdout paths (no --in/--out) ----------------------
"$RCODE" encrypt --ciphers "$FIX/rc123.cps" --pattern inline:132312 <"$FIX/tqbf.txt" >"$TMP/ct" 2>"$TMP/e"
"$RCODE" decrypt --ciphers "$FIX/rc123.cps" --pattern inline:132312 <"$TMP/ct" >"$TMP/pt" 2>"$TMP/e"
...
expect_exit "usage: missing --pattern"  1 "$RCODE" encrypt --ciphers "$FIX/rc1.cps"
expect_exit "usage: unknown flag"       1 "$RCODE" encrypt --ciphers "$FIX/rc1.cps" --pattern inline:1 --bogus
